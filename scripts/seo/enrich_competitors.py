#!/usr/bin/env python3
"""
enrich_competitors.py — v3 competitor enrichment for any SEO client.

For each Status=Proposed row in the client's Competitors DB: fetches the
competitor's top-ranking page (URL identified from priority-keyword SERPs),
pulls DataForSEO authority summary, lists top-20 referring domains,
detects JSON-LD schema types on the page, Places API GBP lookup for local
presence, and Claude synthesizes a competitive angle narrative primed
from the client's own Business Profile.

Enrichment targets (fills EMPTY fields only — never overwrites team edits):
  Competitor Name, Top Ranking Page, Target Cluster, Competing Keywords,
  Content Depth, Uses FAQs, Uses Schema, EEAT Signals, Page Type,
  Strengths, Weaknesses, Referring Domains, Authority Score, Backlinks,
  GBP URL, Review Count, Review Rating, GBP Details, Top Backlinks, Notes

Auto-flips sister-client domains (other RxMedia clients appearing in
SERPs) to Status=Partner with a complementary-service-model note.
Auto-dismisses .gov / .edu / .mil TLDs.

Idempotent. Safe to re-run.

Usage:
    make enrich-competitors CLIENT=lotus_recovery
    make enrich-competitors CLIENT=lotus_recovery DRY=1 LIMIT=2
    python3 scripts/seo/enrich_competitors.py --client x --dry-run
"""
...
# Module-level client context — initialized at the top of main() once the
# --client arg resolves cfg. Functions downstream read from this dict so
# we don't have to thread client state through every call signature.
CTX: dict = {
    "client_key": "",
    "client_name": "",
    "client_domain": "",
    "client_market": "",       # e.g. "Portland" (parsed from canonical_address)
    "client_state": "",        # e.g. "OR" (parsed from canonical_address)
    "positioning": "",         # multi-line string built from Business Profile + cfg
    "sister_domains": set(),   # other RxMedia client domains → Status=Partner
}
...
async def places_find_place(business_name: str, market_hint: str = "") -> dict | None:
    """findplacefromtext — returns place_id if a match exists in the client's market."""
    if not GOOGLE_API_KEY:
        return None
    market = CTX["client_market"]
    state  = CTX["client_state"]
    if not market_hint:
        market_hint = f"{market} {state}".strip()
    query = f"{business_name} {market_hint}" if market_hint else business_name
    ...
    for c in candidates:
        ...
        if (state and state.lower() in addr) or (market and market.lower() in addr):
            return c
    # No regional match — return None so we don't attach the wrong city's office
    return None
...
ANALYSIS_PROMPT = """\
You are analyzing one competitor of {client_name} for the RxMedia SEO battle plan.
...
Ranks in {client_name}'s priority-keyword SERPs at:
{rank_table}
...
{{
  ...
  "weaknesses": "1-2 sentences — where they are thin / where {client_name} could win",
  "competitive_angle": "2-3 sentences — WHY IS THIS A COMPETITOR to {client_name} specifically? Direct service overlap, SERP territory only, adjacent service, cross-geography false positive? What should the team know about competing with them or confirming they are not a real threat worth pursuing?"
}}

If this is clearly a directory / aggregator / state government / wrong-geography site that slipped through filters and is NOT a real competitor, set strengths = "Not a real competitor — [reason]" and competitive_angle = "Recommend Dismissing this entry. [Reason.]".
"""
...
def _init_ctx(client_key: str, cfg: dict) -> None:
    """Populate module-level CTX dict from the client's cfg. Called once at
    the start of main() so downstream functions don't need client args threaded."""
    ...
    # Market + state from canonical_address (e.g. "940 E Ashby Rd., Quinby, SC 29506")
    address = cfg.get("canonical_address") or ""
    market = ""
    state = ""
    if address:
        ...
        if len(parts) >= 2:
            market = parts[-2] if len(parts) >= 3 else ""
            # Extract state from last segment "SC 29506"
            last = parts[-1].strip().split()
            if last:
                state = last[0]

    # Sister client domains — other RxMedia clients whose domains may rank
    sister: set[str] = set()
    for k, c in CLIENTS.items():
        if k == client_key:
            continue
        u = c.get("gsc_site_url") or c.get("website") or ""
        if not u:
            continue
        if "://" not in u:
            u = "https://" + u
    ...
    # Positioning summary — built from vertical(s) + services the client offers.
    # (Future: load Business Profile page dynamically for richer positioning.)
    verticals = cfg.get("vertical") or []
    if isinstance(verticals, str):
        verticals = [verticals]
    vertical_str = ", ".join(verticals) if verticals else "unspecified"
    positioning = (
        f"{cfg.get('name', client_key)} — {market or 'local'}, "
        f"{state or ''} business. Vertical: {vertical_str}.\n"
        f"Address: {address or 'unspecified'}.\n"
        f"(Positioning derived from clients.json; for richer client-specific "
        f"strategic context, see the Business Profile page in Notion.)"
    ).strip()

    CTX["client_key"]     = client_key
    CTX["client_name"]    = cfg.get("name", client_key)
    CTX["client_domain"]  = domain
    CTX["client_market"]  = market
    CTX["client_state"]   = state
    CTX["positioning"]    = positioning
    CTX["sister_domains"] = sister


async def main(client_key: str, limit: int | None, dry_run: bool) -> None:
    from config.clients import CLIENTS
    ...
    print(f"\n── Competitor enrichment v3 for {CTX['client_name']} {'[DRY RUN]' if dry_run else ''} ──")
    print(f"  Client domain: {CTX['client_domain']} | Market: {CTX['client_market']}, {CTX['client_state']}")
    print(f"  Sister-client domains monitored for Partner detection: {len(CTX['sister_domains'])}\n")
    ...
    # Phase 2a: auto-flip any Sister RxMedia client domain → Status=Partner
    print("\n[3/6] Flipping sister-client domains to Status=Partner...")
    enrich_rows: list[dict] = []
    for row in rows:
        domain = domain_of(_plain_url(row["properties"].get("Website", {})))
        if domain and domain in CTX["sister_domains"]:
            reason = (
                f"NOT A COMPETITOR — sister RxMedia client (domain {domain} matches "
                f"another client in our roster). Complementary service relationship; "
                f"downstream agents treat Status=Partner as OUT-of-competitor-set but "
                f"IN-play for interlink suggestions (team-approved only)."
            )
            print(f"  ✓ partner: {domain}")
            await flip_partner(notion, row, reason, dry_run)
        else:
        ...
        if place and place.get("place_id"):
            ...
            if details:
                ...
                if place_domain and place_domain != domain and not place_domain.endswith("." + domain) and not domain.endswith("." + place_domain):
                    print(f"     GBP: {CTX['client_state']} match found but website mismatch ({place_domain} != {domain}) — discarding")
                else:
            ...
            print(f"     GBP: no {CTX['client_state']}-market match")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Competitor v3 enrichment — fill empty fields on Proposed rows")
    ...
    asyncio.run(main(client_key=args.client, limit=args.limit, dry_run=args.dry_run))
